...
local TabNames = {"Me", "Visual", "Troll", "Misc", "Settings"}
local TabBtns = {}
local TabFrames = {}
...
Toggle(MeTab, "Invisible", false, 9, function(on)
    ...
    if on then
        ...
        if char then
            ...
            if hum then
                if hum:FindFirstChild("Health") then hum.Health.Transparency = 1 end
                if hum:FindFirstChild("HealthBar") then hum.HealthBar.Transparency = 1 end
                if hum:FindFirstChild("HealthBarBackground") then hum.HealthBarBackground.Transparency = 1 end
            end
        end
        if not InvisibleConn then
...
Toggle(MeTab, "Click TP", false, 10, function(on)
...
Section(SetTab, "KEYBINDS", 1)

local menuKb = Keybind(SetTab, "Toggle Menu", Config.ToggleKey, 2, function(k) Config.ToggleKey = k end)
local speedKb = Keybind(SetTab, "Speed (Hold)", Config.Hotkeys.Speed, 3, function(k) Config.Hotkeys.Speed = k end)
local jumpKb = Keybind(SetTab, "Jump Power (Hold)", Config.Hotkeys.JumpPower, 4, function(k) Config.Hotkeys.JumpPower = k end)
local gravKb = Keybind(SetTab, "Gravity", Config.Hotkeys.Gravity, 5, function(k) Config.Hotkeys.Gravity = k end)
local flyKb = Keybind(SetTab, "Fly", Config.Hotkeys.Fly, 6, function(k) Config.Hotkeys.Fly = k end)
local espKb = Keybind(SetTab, "ESP", Config.Hotkeys.ESP, 7, function(k) Config.Hotkeys.ESP = k end)
local fbKb = Keybind(SetTab, "Fullbright", Config.Hotkeys.Fullbright, 8, function(k) Config.Hotkeys.Fullbright = k end)
local blKb = Keybind(SetTab, "Bloom", Config.Hotkeys.Bloom, 9, function(k) Config.Hotkeys.Bloom = k end)
local ttKb = Keybind(SetTab, "Target Tool", Config.Hotkeys.TargetTool, 10, function(k) Config.Hotkeys.TargetTool = k end)

Section(SetTab, "INFO", 11)

Btn(SetTab, "Destroy Menu", 12, function()
